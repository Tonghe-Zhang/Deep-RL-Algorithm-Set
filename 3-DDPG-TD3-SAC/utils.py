...
def get_log_dict(agent_name, manager=None, num_seeds=0):
    log_keys = ['critic_loss', 'actor_loss', 'td_error',
                'eval_steps', 'eval_returns', 'train_steps', 'train_returns']
    ...
    if manager is None:
        ...
        return manager.dict({key: manager.list([[]] * num_seeds)
                             for key in log_keys})
...
def plot_scores(scores, steps=None, window=100, label=None, color=None):
    ...
    if len(scores) > 1:
        ...
        # score low and score high: mean -+ standard deviation.
        ...
        # how to plot the region between two lines.
        # alpha is transparency level
        # where (optional): is A boolean array that is the same length as x. 
        # It allows for selective filling where the condition is True. If None, the fill is applied everywhere.
