...
class PPOReplayBuffer(ReplayBuffer):
    ...
    def compute_advantages_and_returns(self, agent) -> None:
        ...
        for t in reversed(range(self.capacity)):
            ...
            # Hint: you can calculate step t's advantage using step t + 1's advantage
            ############################
            # Rememer to add 1-self.done[t] when calculating delta and advantage.
            # also notice that we are calculating the time series of {A_t}
            """
            next_values=torch.Size([16]), 
            self.reward=torch.Size([1024, 16]),
            self.done=torch.Size([1024, 16]),
            self.value=torch.Size([1024, 16])
            """
            
            next_values = self.get_next_values(agent, t)
            delta=self.reward[t]+self.gamma*next_values*(1-self.done[t])-self.value[t]
            last_gae_lam=delta+self.gamma*self.gae_lambda*last_gae_lam*(1-self.done[t])
            self.advantage[t]=last_gae_lam
            ############################
        # calculate the returns
        """
        notice that the advantage in ppo is not really definned by the Bellman eqution. 
        It is the finite truncations of \lambda GAE.
        it is not even a function os (s,a) but only the function of s, just like value function.
        So we can add them together to define returns=V_{target}
        """
