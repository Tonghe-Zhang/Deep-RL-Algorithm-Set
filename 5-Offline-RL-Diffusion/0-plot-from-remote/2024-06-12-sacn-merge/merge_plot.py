...
def compare(syn_ratios:list[float], max_normalized_returns:list[float]):
    # Data
    ...
    # Annotate the points
    for i, j in zip(syn_ratios, max_normalized_returns):
...
def plot_returns_loss(syn_ratios=[0.2,0.4,0.6,0.8]):
    ...
    plt.title('Effect of Mixing with Synthetic Dataset on EDAC Algorithm')
    plt.legend()
    ...
    plt.subplot(1,3,1)
    for i, (syn_ratio,data) in enumerate(data_pct.items()):
        plt.plot(data['timestep'], data['loss/actor'], label=''+str(syn_ratio*100)+'%', color=colors[i])
    plt.xlabel('Step')
    ...
    plt.subplot(1,3,2)
    for i, (syn_ratio,data) in enumerate(data_pct.items()):
        plt.plot(data['timestep'], data['loss/critics'], label=''+str(syn_ratio*100)+'%', color=colors[i])
    plt.xlabel('Step')
    ...
    plt.subplot(1,3,3)
    for i, (syn_ratio,data) in enumerate(data_pct.items()):
        plt.plot(data['timestep'], data['loss/alpha'], label=''+str(syn_ratio*100)+'%', color=colors[i])
    plt.xlabel('Step')
